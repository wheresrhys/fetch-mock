...
`fetch-mock` has a powerful API to cover all sorts of use cases, but most cases can be covered by usig only a relatively small subset of the API. These docs cover the minimum you will need to know to be productive. Power users my also refer to the [complete API docs](#power-users)
